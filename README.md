...
Директива `ssi: on;` позволяет работать с SSI, а блоки `upstream` и `location` обеспечивают корректный роутинг для url-ссылок, например url, начинающийся на `/blue` будет работать с `team_blue:3001`. Кроме того, корневой путь `/` завязан на команде красных, отвечающей за продукт и странице в целом.