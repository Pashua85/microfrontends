...
Селекторы для выбора вариантов сейчас фактически являются сейчас ссылками и каждый клик по ним ведет к перезагрузке страницы. В терминале справа видно, как запрос сначала попадает к красной команде, отвечающей за страницу продукта, а затем в разметку добавляются фрагменты от синей и зеленой команд.

Если обратно включить Javascript, то будут видны логи только первого запроса.
Все последующие изменения моделей тракторов будут обрабатываться только на клиенте, точно также как и в первом примере. В последующем примере данные о продукте будут удалены из скрипта страницы и будут загружаться, как и положено, через REST api.
